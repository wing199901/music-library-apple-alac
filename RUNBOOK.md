...
| Continuous import | Lidarr Custom Script `scripts/m_to_alac.py` (On Release Import / On Upgrade) writes the same relative path under `ALAC_ROOT` |
...
```text
SRC = M:\          (Linux/Docker: /music)
DST = D:\Music-ALAC  (Linux/Docker: /music-alac)
...
One CLI: [scripts/m_to_alac.py](./scripts/m_to_alac.py).
...
Env: `MASTER_ROOT`, `ALAC_ROOT`, `LOG_FILE` (default `$ALAC_ROOT/_m_to_alac.log`), `COVER_ART_ARCHIVE`.
...
`scripts/m_to_alac.py` runs this after each convert (bulk and Lidarr).
...
See `docs/adr/0001`–`0007` (ALAC on `D:`, full convert before Apple, ffmpeg ALAC, `M:\` overlay, verify before Apple, Add Folder + cover embed, Lidarr On Import / one Python CLI).
