#!/usr/bin/env bash
# Synthetic-audio tests for scripts/m_to_alac.py (no copyrighted music).
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
SCRIPT="$ROOT/scripts/m_to_alac.py"
PY="${PYTHON:-python3}"
PASS=0
FAIL=0
...
WORKDIR="$(mktemp -d /tmp/m-to-alac-test.XXXXXX)"
...
print_kind() {
  local file="$1"
  MASTER_ROOT="$MASTER" ALAC_ROOT="$ALAC" LOG_FILE="$LOG" COVER_ART_ARCHIVE=0 \
    "$PY" "$SCRIPT" --print-cover "$file" | awk -F '\t' '{print $1}'
}

echo "== Test event =="
set +e
lidarr_eventtype=Test "$PY" "$SCRIPT"
te=$?
set -e
...
echo "== Lidarr CLI flags =="
p8c="$MASTER/Artist/P8c"
make_flac "$p8c/c.flac"
run --event AlbumDownload --added-tracks "$p8c/c.flac"
assert "p8c-cli" test -f "$ALAC/Artist/P8c/c.m4a"
...
PATH="$fakebin:$PATH" MASTER_ROOT="$MASTER" ALAC_ROOT="$ALAC" LOG_FILE="$LOG" \
  ...
  "$PY" "$SCRIPT" --print-cover "$p5/song.flac" >"$WORKDIR/p5.out"
...
PATH="$fakebin:$PATH" MASTER_ROOT="$MASTER" ALAC_ROOT="$ALAC" LOG_FILE="$LOG" \
  COVER_ART_ARCHIVE=1 CURL_BIN="$fakebin/curl" \
  "$PY" "$SCRIPT" --print-cover "$p5b/song.flac" >"$WORKDIR/p5b.out"
assert_eq "p5b-kind-none" "$(awk -F '\t' '{print $1}' "$WORKDIR/p5b.out")" "none"

echo "== CAA via --release-mbid flag =="
p5c="$MASTER/Artist/P5c"
make_flac "$p5c/song.flac"
PATH="$fakebin:$PATH" MASTER_ROOT="$MASTER" ALAC_ROOT="$ALAC" LOG_FILE="$LOG" \
  COVER_ART_ARCHIVE=1 CURL_BIN="$fakebin/curl" \
  "$PY" "$SCRIPT" --release-mbid "$mbid" --print-cover "$p5c/song.flac" >"$WORKDIR/p5c.out"
assert_eq "p5c-kind" "$(awk -F '\t' '{print $1}' "$WORKDIR/p5c.out")" "cover-art-archive"

echo "== Refuse ALAC into master root =="
set +e
MASTER_ROOT="$MASTER" ALAC_ROOT="$MASTER" LOG_FILE="$LOG" COVER_ART_ARCHIVE=0 \
  "$PY" "$SCRIPT" "$album/01 Track.flac"
refuse=$?
set -e
...
balac="$bulk/a"
mkdir -p "$bmaster/X"
make_flac "$bmaster/X/t.flac"
"$PY" "$SCRIPT" "$bmaster" "$balac"
assert "bulk-two-arg" test -f "$balac/X/t.m4a"
assert_eq "bulk-two-arg-codec" "$(codec_of "$balac/X/t.m4a")" "alac"
